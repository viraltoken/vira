...
{    
    if(block)
	{
        ...
        if (pindex->IsProofOfStake()) { 
            ...
            ui->moneySupplyBox->setText(QString::number(getMoneySupply(height), 'f', 6) + " VIRA");
    } 
    
    else {
        std::string txid = ui->txBox->text().toUtf8().constData();
        ui->valueBox->setText(QString::number(getTxTotalValue(txid), 'f', 6) + " VIRA"); 
        ...
        ui->feesBox->setText(QString::number(getTxFees(txid), 'f', 6) + " VIRA"); 
    }
}
