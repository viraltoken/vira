    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Viral address (e.g. P1O3GFgK3KtzaV9ce467nq5m5Vis1E4hZ)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
		...
		{ 
#if QT_VERSION < 0x050000 
        formatted.append(tr("<b>%1</b> in %4 blocks of %5 VIRA each to %2 (%3)?").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount),  
			Qt::escape(rcp.label),  
			rcp.address,  
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Viral address"));
        }
        else
        {
